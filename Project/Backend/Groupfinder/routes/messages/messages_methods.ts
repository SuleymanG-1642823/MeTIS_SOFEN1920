...
/**
 * Manage all interactions with messages in the database
 */
export class MessageController {
     ...
     */
    public getConversation(userID1: number, userID2: number): Promise<Message[]>{
        return new Promise(
            (resolve, reject) => {
     ...
     */
    public getCorrespondents(userID: number): Promise<number[]> {
        return new Promise(
     ...
     */
    public addMessage(msg: Message): Promise<number> {
        return new Promise(
            (resolve: any, reject: any) => {
                ...
                db_conn.query(query, params, async (err: any, rows: any) => {
                    if (err) {
                        ...
                        try{
                            const messageID: number = await this.getMessageId(msg);
                            resolve(messageID);
                        } catch (err) {
                            reject(err);
                        }                
     ...
     */
    public getMessageId(message: Message): Promise<number> {
        return new Promise(
            (resolve, reject) => {
